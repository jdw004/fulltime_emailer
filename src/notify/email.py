...
def build_subject(jobs: list[Job], grouped, prefix: str, faang: list[Job] | None = None) -> str:
    if faang:
        ...
        return f"🚨 FAANG job out now — {names} ({len(jobs)} new full-time job(s))"
    return f"{prefix} {len(jobs)} new full-time job(s) — {_counts_summary(grouped)}"
...
def build_html(grouped: list[tuple[str, list[Job]]], total: int, faang: list[Job] | None = None) -> str:
    headline = "🚨 FAANG job out now" if faang else "🚀 Daily Full-Time Digest"
    rows = []
    if faang:
        rows.append(_faang_banner(faang))
    rows.append(
        f"<p style='font:14px system-ui,sans-serif;color:#444'>"
        f"<b>{total}</b> new full-time job posting(s) matched your filters.</p>"
    )
    for cat, jobs in grouped:
    ...
    return (
        ...
        "Generated by fulltime_pos_emailer. Tune sources/filters in <code>config/</code>.</p>"
        "</div>"
    )
...
def build_text(grouped: list[tuple[str, list[Job]]], total: int, faang: list[Job] | None = None) -> str:
    ...
    if faang:
        ...
        lines.append("")
    lines += [f"{total} new full-time job posting(s):", ""]
    for cat, jobs in grouped:
...
def send_email(
    ...
    *,
    ...
    subject = subject_override or build_subject(
        jobs, grouped, email_cfg.get("subject_prefix", "[Full-Time Alert]"), faang
    )
