...
def test_subject_urgent_when_faang():
    ...
    subj = E.build_subject(jobs, grouped, "[Full-Time Alert]", E.faang_jobs(jobs, FAANG))
...
def test_subject_normal_without_faang():
    jobs = [_job("Acme")]
    grouped = E.group_by_category(jobs, ["swe"])
    subj = E.build_subject(jobs, grouped, "[Full-Time Alert]", [])
    assert subj.startswith("[Full-Time Alert]")
